        ...
        <div className="flex items-center justify-between gap-4 py-2">
          <div className="flex items-center gap-1">
            {navItems.map((item) => {
              ...
              } else if (item.href === '/weekplanning/cap-overzicht') {
                isActive = pathname?.startsWith('/weekplanning') ?? false
              } else if (item.href === '/calibratie') {
                isActive = pathname?.startsWith('/calibratie') ?? false
              } else {
                isActive = pathname?.startsWith(item.href) ?? false
              }
              const baseClasses = 'flex items-center gap-2 px-4 py-2 text-sm font-semibold transition-colors border-b-2'
